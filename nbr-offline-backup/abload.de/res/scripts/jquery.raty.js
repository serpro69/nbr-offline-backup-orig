...
(function($) {
	...
	$.fn.raty = function(settings) {
		...
		if (options.path.substring(options.path.length - 1, options.path.length) != 'https://abload.de/') {
			options.path += 'https://abload.de/';
		}
		...
		if (!options.readOnly) {
			if (options.showCancel) {
				...
				$this.css('width', options.number * 20 + 20 + 20);
	...
	$.fn.raty.defaults = {
		...
		cancelPlace:	'left',
		hintList:		['nicht gut', 'eher nicht gut', 'durchschnittlich', 'gut', 'sehr gut'],
		number:			5,
		path:			'res/imgs/',
		readOnly:		false,
		scoreName:		'score',
